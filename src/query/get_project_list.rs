...
pub struct GetProjectListQuery<Str = String> {
  pub base: QueryBase<Str>,
...
pub type GetProjectListQueryView<'req> = GetProjectListQuery<&'req str>;

impl<Str: AsRef<str>> GetProjectListQuery<Str> {
  pub const fn new(base: QueryBase<Str>) -> Self {
    Self {
      base,
      archived: None,
      id_after: None,
  ...
  pub fn as_view(&self) -> GetProjectListQueryView<'_> {
    GetProjectListQueryView {
      base: self.base.as_view(),
